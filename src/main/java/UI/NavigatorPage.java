...
public class NavigatorPage {
	...
	{ 
		...
		try
		{
			User.cards.items=LedgerList.getEntries(DBUtil.query(User.getLoginAs(), "card", "card"));
			User.earnings.items=LedgerList.getEntries(DBUtil.query(User.getLoginAs(), "earnings", "card"));
			User.expenses.items=LedgerList.getEntries(DBUtil.query(User.getLoginAs(), "expense", "card"));	
		}
	...
	public NavigatorPage() {
		init();
		if(User.currBudget.endDate.compareTo(LocalDate.now())>=0)
		{ 
